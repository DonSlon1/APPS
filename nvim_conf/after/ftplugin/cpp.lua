...
vim.opt_local.path:append("/usr/include/c++/**")

-- Folding settings
vim.opt_local.foldmethod = "syntax"
vim.opt_local.foldlevel = 99

-- Auto-insert header guards for .hpp files
local group = vim.api.nvim_create_augroup("CppHeaderGuard", { clear = true })
vim.api.nvim_create_autocmd("BufNewFile", {
  group = group,
  pattern = {"*.hpp", "*.hxx", "*.hh"},
  callback = function()
    local filename = vim.fn.expand("%:t:r"):upper()
    local guard = filename .. "_HPP_"
    local lines = {
      "#ifndef " .. guard,
      "#define " .. guard,
      "",
...
vim.keymap.set("n", "<F6>", ":w<CR>:!g++ -std=c++17 -Wall -Wextra -g % -o %:r && ./%:r<CR>",
  { buffer = true, desc = "Compile and run C++ file" })
vim.keymap.set("n", "<F7>", ":w<CR>:!g++ -std=c++17 -Wall -Wextra -g % -o %:r<CR>",
  { buffer = true, desc = "Compile C++ file" })
vim.keymap.set("n", "<F8>", ":!./%:r<CR>",
  { buffer = true, desc = "Run compiled C++ file" })

-- Quick access to man pages and cppreference
vim.keymap.set("n", "K", function()
  local word = vim.fn.expand("<cword>")
  vim.cmd("Man 3 " .. word .. " || Man " .. word)
